...
Time ProcessUSART(char *string) // Processing the command via USART
{
  Time time;
  ...
  {
    if (min == NULL)
      printf("HELP:format | HELP:range | HELP:STOP\r\n");
      ...
      printf("%c", znak); // Vymazat a nastavit local line editing a local echo v terminal v putty
	    if (znak == '\r' || znak == '\n')
	    {
	      if (index == 0)
	        continue;
          string[index] = '\0';
          time = ProcessUSART(string); // Set values in RTC registers
          SetTime(time);
	      index = 0;
	    }
	    else
